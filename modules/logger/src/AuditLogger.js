...
class AuditLogger { 
  ...
  constructor(incomingReqHeaders, filename = __filename) {
    ...
    this.data = incomingReqHeaders.options ? incomingReqHeaders.options.data : {};
  ...
  _getTraceInfo(obj) {
    ...
    return (
        `${process.pid} [NODE-Thrd-BpRImg] ${path.parse(info.filename || __filename).name} ` +
        `${obj.level} [${this.AppName}, ` +
        `${info.traceId || '<No Request Context>'}, ${info.spanId || '<No Request Context>'}, false] ${obj.message}`
      );
  }
  ...
  auditEnd(res) {
    ...
    if (!this.startTime)
      throw new Error('Audit not started!!');
    ...
    let message = `${process.pid} [NODE-Thrd-${this.appName}] ${this.filename} DEBUG [${this.appName},${req.traceId},${req.spanId}]},false]`;
        ...
        message = `${message} queryString={${Object.keys(req.query).length === 0 ? {} : JSON.stringify(req.query)}}, headerMap={${prettyPrintObject(req.headers)}},`;
